...
async function main() {
  ...
  const worker = app.get(ReportExportWorkerService);
  const workerId = process.env.JOB_WORKER_ID ?? `report-export-worker-${process.pid}`;
  const batchSize = readPositiveInt('JOB_BATCH_SIZE', 3);
